<div class="tab-pane fade register_viettel" id="tab2danger">
    <div class="row">
        <?php if (isset($webSignUp) && isset($websiteData) && ($webSignUp['Viettel Mobile'] !== NULL) && (count($webSignUp['Viettel Mobile']['list_service']) > 0)): ?>
            <?php foreach ($webSignUp['Viettel Mobile']['list_service'] as $key_service => $service): ?>
                            ...
                            <div style="text-align: center;">
                                <ul class="list-link-register">
                                    <?php foreach ($service['list_package'] as $key_package => $package): ?>
                                        <li>
                                            <a href="<?php echo $websiteData['site_url'] . trim($webSignUp['Viettel Mobile']['link_sign_up'] . strtoupper($key_package)); ?>" class="btn btn-success" title="Đăng ký <?= $package['name']; ?> dịch vụ <?= $service['name']; ?>"><?= $package['name']; ?></a>
                                        </li>
                                    <?php endforeach; ?>
                            ...
                            <ol>
                                <?php foreach ($service['list_package'] as $key_package => $package): ?>
                                    <li><?= $package['name']; ?>: soạn <strong><?= $package['mo']; ?></strong> gửi
                                        <strong><?= $webSignUp['Viettel Mobile']['shortcode']; ?></strong>. Giá cước:

<div class="tab-pane fade register_vnmb" id="tab3danger">
    <div class="row">
        <?php if (isset($webSignUp) && isset($websiteData) && ($webSignUp['Vietnamobile'] !== NULL) && (count($webSignUp['Vietnamobile']['list_service']) > 0)): ?>
            <?php foreach ($webSignUp['Vietnamobile']['list_service'] as $key_service => $service): ?>
                            ...
                            <div style="text-align: center;">
                                <ul class="list-link-register">
                                    <?php foreach ($service['list_package'] as $key_package => $package): ?>
                                        <li>
                                            <a href="<?php echo $websiteData['site_url'] . trim($webSignUp['Vietnamobile']['link_sign_up'] . strtoupper($key_package)); ?>" class="btn btn-success" title="Đăng ký <?= $package['name']; ?> dịch vụ <?= $service['name']; ?>"><?= $package['name']; ?></a>
                                        </li>
                                    <?php endforeach; ?>
                            ...
                            <ol>
                                <?php foreach ($service['list_package'] as $key_package => $package): ?>
                                    <li><?= $package['name']; ?>: soạn <strong><?= $package['mo']; ?></strong> gửi
                                        <strong><?= $webSignUp['Vietnamobile']['shortcode']; ?></strong>. Giá cước:
